...
var resizePizzas = function(size) {
  ...
  // Iterates through pizza elements on the page and changes their widths -- Created scope outside of loop
  function changePizzaSizes(size) {
    var rdmPizCont =document.querySelectorAll(".randomPizzaContainer");
    var randomlength = rdmPizCont.length;

    var ddx =determineDx(rdmPizCont)
    for (var i = 0; i < randomlength; i++) {
      var dx = (ddx[i], size);
      var newwidth = (rdmPizCont[i].offsetWidth + dx) + 'px';
      drdmPizCont[i].style.width = newwidth;
    }
  }
...
you spot and fix both?
Built into the code, you'll find a few instances of the User Timing API
(window.performance), which will be console.